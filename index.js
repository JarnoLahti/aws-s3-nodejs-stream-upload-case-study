...
if (!process.env.AWS_BUCKET_NAME) {
  throw Error("NO 'AWS_BUCKET_NAME' DEFINED IN ENVIRONMENT: define it in .env file which should be found from root of this project");
}
...
const s3Upload = async (fileStream, fileName) => {
  await s3.upload({ Bucket: AWS_BUCKET_NAME, Key: fileName, Body: fileStream }, { partSize: 10 * 1024 * 1024 }).promise();
};
...
const handleGetObjects = async (res) => {
  const getResult = await s3.listObjects({ Bucket: AWS_BUCKET_NAME, MaxKeys: 10 }).promise();
...
const handleDeleteObject = async (id, res) => {
  await s3.deleteObject({ Bucket: AWS_BUCKET_NAME, Key: id }).promise();
  res.end();
};
